...
func (s Server) Routes() {
	...
	s.Router.Handle(http.MethodGet, "/snippet/display/:id", handlers.RecoverPanic(handlers.SecureHeaders(s.HandleDisplaySnippet())))
	s.Router.Handle(http.MethodPost, "/snippet/create", handlers.RecoverPanic(handlers.SecureHeaders(s.HandleCreateSnippet())))
	s.Router.Handle(http.MethodGet, "/snippet/create", handlers.RecoverPanic(handlers.SecureHeaders(s.HandleShowSnippetForm(FormVals{Snippet: models.NewEmptySnippet()}))))
